...
// label and imageUrl are defined in schema.prisma
const formSchema = z.object({
  name: z.string().min(1),
  images: z.object({ url: z.string() }).array(),
  price: z.coerce.number().min(1), // We add coerce because we are working with a Decimal type
...
interface ProductFormProps {
  initialData:
    ...
    | null; // We extend the Product type because we are also bringing the images
  categories: Category[];
  colors: Color[];
  sizes: Size[];
}
...
const ProductForm: React.FC<ProductFormProps> = ({ initialData, categories, colors, sizes }) => {
  ...
  // The following state will control the alert (delete) modal
